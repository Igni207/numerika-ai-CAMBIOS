...
- **Motor de IA (`ai.js`):** Transición exitosa a la IA de OpenAI (`gpt-4o-mini`). Se implementó un promting específico para inyectar dinámicamente el contexto de la página (ej. simuladores o solvers) y asumir el rol de tutor.
...
- **`ComparisonPage.jsx`:** Implementada para ejecutar simultáneamente hasta 4 métodos numéricos cerrados y abiertos. Genera un reporte comparativo automático analizado pedagógicamente por la IA.
...
4. **Protección de Tokens de IA (`server.js`):** La base de datos antes devolvía el 100% de la tabla del usuario con sus mensajes históricos pasados a IKA. Esto consumía exponencialmente el contexto de la API sumando altísimos costos. Ahora se truncan y limitan estáticamente los últimos `50 mensajes`.
5. **Escape a Función Rate Limit (`ai.js`):** Solucionado un caso borde donde el backoff de intentos de la IA retornaba de forma silente variables `undefined`. Ahora genera un error controlado en interfaz.
